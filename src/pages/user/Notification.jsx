        ...
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-[#64748B]">
              Stay updated on your bookings, saved stays,
              messages and important EliteBNB account activity.
            </p>
          </div>
        ...
        {notifications.length === 0 ? (
          <div className="rounded-2xl border border-[#E5E7EB] bg-white p-12 text-center shadow-sm">
            ...
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#64748B]">
              When your bookings, trips, wishlist or account
              need attention, you'll see it here.
            </p>
          </div>
        ) : (
...
/**
 * Renders the correct notification symbol without creating a dynamic component
 * variable during render, which keeps React compiler lint happy.
 */
function NotificationTypeIcon({ type }) {
  switch (type) {
    case "NEW_BOOKING":
    case "BOOKING_REQUESTED":
      return <CalendarDays size={21} aria-hidden="true" />;

    case "BOOKING_CONFIRMED":
    case "BOOKING_COMPLETED":
      return <CheckCircle2 size={21} aria-hidden="true" />;

    case "BOOKING_CANCELLED":
      return <XCircle size={21} aria-hidden="true" />;

    default:
      return <Bell size={21} aria-hidden="true" />;
  }
}
